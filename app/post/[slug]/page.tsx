...
// Extract h2/h3 headings from HTML body for TOC
...
const MostReadWidget = ({ posts }: { posts: RelatedPost[] }) => {
  ...
  const bn = (n: number) => n.toString().replace(/\d/g, x => "০১২৩৪৫৬৭৮৯"[+x]).padStart(2, "০");
  return (
    <div className="mb-6 border border-border overflow-hidden">
      <div className="bg-primary px-4 py-2.5 border-b-2 border-gold">
    ...
    <>
              ...
              <div className="flex gap-1.5 ml-auto">
                {available.map((l) => (
                  <button key={l} onClick={() => setLang(l as LangCode)}
                    className={`text-[11px] px-3 py-1 border transition-colors ${langClass[l]} ${lang === l ? "bg-gold border-gold text-primary font-bold" : "border-white/25 text-white/60 hover:border-gold hover:text-gold"}`}>
      ...
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-8 flex gap-8 items-start">
        ...
        <main className="flex-1 min-w-0">
          ...
          {/* Article card */}
          <article
            className="bg-card border border-border p-7 sm:p-10 mb-6"
          ...
          {relatedPosts.length > 0 && (
            <div className="mb-8">
              <div className="bg-primary px-4 py-3 border-t-[3px] border-gold mb-0">
