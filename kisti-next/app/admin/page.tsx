    ...
    <div>
      <div className="flex items-center justify-between mb-10">
        ...
        <Button asChild className="bg-foreground text-background hover:bg-foreground/90 rounded-none">
          <Link href="/admin/posts/new"><Plus className="w-4 h-4 mr-2" /> New post</Link>
        </Button>
      </div>
      ...
      {loading ? (
        ...
        <div className="text-center py-20 border border-dashed border-border">
          ...
          <Button asChild><Link href="/admin/posts/new">প্রথম পোস্ট লিখুন</Link></Button>
        </div>
      ) : (
            ...
            return (
              <div key={p.id} className="flex items-center justify-between px-5 py-4 border-b border-border last:border-0 hover:bg-secondary/30">
                ...
                <div className="flex gap-2">
                  <Button variant="ghost" size="icon" asChild><Link href={`/admin/posts/${p.id}`}><Pencil className="w-4 h-4" /></Link></Button>
